...
NDVI_PATTERN = re.compile(
    r".+_NDVI_(\d{4}-\d{2}-\d{2})_to_(\d{4}-\d{2}-\d{2})\.tif"
)

def tif_covers_date(filename, selected_date):
    m = NDVI_PATTERN.match(filename)
    if not m:
        return False

    start = datetime.strptime(m.group(1), "%Y-%m-%d").date()
    end = datetime.strptime(m.group(2), "%Y-%m-%d").date()
    return start <= selected_date <= end
...
def zonal_ndvi_mean(tif_path, geometry):
...
def regional_ndvi():
    ...
    level = payload["level"]        # district | tehsil
    state = payload.get("state")
    selected_date = datetime.strptime(payload["date"], "%Y-%m-%d").date()

    geojson_path = (
        "static/india_district.geojson"
        if level == "district"
        else "static/india_taluk.geojson"
    )
    ...
    # find all valid tiffs for date
    tiffs = [
        os.path.join(NDVI_DIR, f)
        for f in os.listdir(NDVI_DIR)
        if tif_covers_date(f, selected_date)
    ]
    ...
    for feature in geojson["features"]:
        ...
        if state and props["NAME_1"] != state:
            continue

        name = (
            props["NAME_2"] if level == "district"
            else props["NAME_3"]
        )
        ...
        for tif in tiffs:
            val = zonal_ndvi_mean(tif, geom)
            if val is not None:
                ndvi_values.append(val)

        mean_ndvi = (
            round(float(np.mean(ndvi_values)), 3)
